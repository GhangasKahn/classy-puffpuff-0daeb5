...
**Tier:** Specialist (Hermes-class default, pack-only until wired). Escalate to Brain when open markers conflict with operator urgency on capital-touching asks, or when excommunicado-path (ToS/replica/RA/scrape) is requested.
...
**Desk JS:** pack-only `wick` (will be wired in playground catalog + `lpros-command/src/playground/runner.js` PACK_ONLY / persona path). Until wired, `packOnly: true`. Desk JS is not live. Do not claim a worker ran.

**Hard boundaries:**
- Pack-only until wired; never claim live Browse
